...
```bash
./generate-certs.sh 192.168.100.11
```
...
openssl req -x509 -newkey rsa:4096 \
    ...
    -days 365 \
    -nodes \
  -subj "/C=US/ST=State/L=City/O=MobileCameraReceptor/CN=192.168.100.11" \
  -addext "subjectAltName=DNS:localhost,DNS:127.0.0.1,IP:127.0.0.1,IP:192.168.100.11,IP:0.0.0.0"
```
...
- **Frontend**: `https://192.168.100.11:3000`
- **Backend API**: `https://192.168.100.11:3001`
- **WebSocket**: `wss://192.168.100.11:3001`
...
BACKEND_PORT=3001
MEDIASOUP_ANNOUNCED_IP=192.168.100.11
...
# NDI Bridge
BACKEND_URL=https://192.168.100.11:3001
BACKEND_WS_URL=wss://192.168.100.11:3001
```
...
1. **Backend health check**:
   ```bash
   curl -k https://192.168.100.11:3001/health
   ```

2. **Frontend access**:
   Open `https://192.168.100.11:3000` in browser
